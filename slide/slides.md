...
**1. Định kiến tiềm ẩn (bias)**
Agent controller (GPT-4o) mang định kiến tri thức riêng.
*VD: model có thể ưu tiên nguồn tiếng Anh, bỏ sót context văn hoá khác.*

**2. Tri thức tĩnh, không cập nhật động**
Agent chỉ dùng tri thức đã học lúc train.
*VD: claim về sự kiện mới sau training cutoff, model không có evidence để đối chiếu.*

</div>
<div class="col">

**3. Thiếu cơ chế cải thiện mô hình**
Framework chỉ kiểm định, chưa giúp model học lại.
*VD: phát hiện LLaMA yếu ở "multi-step reasoning" nhưng không có bước fine-tune khắc phục.*
...
Nhóm khai thác **hạn chế (2)**: paper tự đề xuất RAG, Table 3 cho thấy `[evidence]` dễ hơn `[claim]` rõ rệt, đủ cơ sở để thử nghiệm.
